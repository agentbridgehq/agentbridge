...
| **VS Code / Copilot** | `~/Library/Application Support/Code/User/agent-plugins`, overridable with `--agent-plugins-dir` or `$VSCODE_AGENT_PLUGINS` | **Read out of the shipped code, and it did not work.** `agentPluginsHome` is built beside `mcp.json` in the same profile directory, and the directory is created there — but a package placed in it was not listed when VS Code was asked what skills it had. Either the path is not the whole story or the feature is gated. Nothing is written there |
